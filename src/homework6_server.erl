...
handle_call({lookup, TableName, Key}, _From, State) ->
    case ets:lookup(TableName, Key) of
        [] -> 
            {reply, undefined, State}; % Якщо ключ не знайдено
        [{_, Value, infinity}] -> 
            {reply, Value, State}; % Дані без TTL
        [{_, Value, Expiration}] -> 
            CurrentTime = erlang:system_time(second),
            if
                Expiration > CurrentTime -> 
                    {reply, Value, State}; % Якщо TTL ще не минув
                true -> 
                    ets:delete(TableName, Key), % Видалити застарілі дані
